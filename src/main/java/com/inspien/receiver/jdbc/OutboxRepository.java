...
public class OutboxRepository {
    ...
    public int[] batchInsert(List<Outbox> outboxes) {
        String sql = """
                ...
                (APPLICANT_KEY, ORDER_ID, USER_ID, ITEM_ID, NAME, ADDRESS, ITEM_NAME, PRICE, STATUS, UPDATED, PROCESSED)
                VALUES
                (:applicantKey, :orderId, :userId, :itemId, :name, :address, :itemName, :price, :status, :updated, :processed)
                """;
    ...
    public List<Outbox> findUnprocessed(String applicantKey) {
        String sql = """
                SELECT APPLICANT_KEY, ORDER_ID, USER_ID, ITEM_ID, NAME, ADDRESS, ITEM_NAME, PRICE, STATUS, UPDATED, PROCESSED
                FROM OUTBOX_TB
    ...
    public int deleteProcessed(String applicantKey) {
        String sql = """
                ...
                WHERE  APPLICANT_KEY = :applicantKey AND PROCESSED = true
        """;
        MapSqlParameterSource params = new MapSqlParameterSource()
                .addValue("applicantKey", applicantKey);
        return template.update(sql, params);
